...
function ScrapBook(props) {
    ...
    const [srapbookPhotos, setSrapbookPhotos] = useState([]);

    const getScrapbookPhotos = async (URL) => {
            ...
            filter: {
                ...
                filters: [
                    ...
                    },
                    {
                        field: "mediaCategory",
                        operator: "eq",
                        value: MEDIA_CATEGORY.SCRAPBOOK
                    }
    ...
    useEffect(() => {
        getScrapbookPhotos(AVTAR.GET_AVATAR_MEDIA)
    }, [])
    if (isLoading) {
        return <>
    ...
    return (
        <>
            <div className="gallery-grid mb-3">
                {srapbookPhotos.length > 0
                    ? srapbookPhotos.map((mediaItem, index) => (
            ...
            <Link to={`${PUBLIC_ROUTES_SLUGS.AVATORS}/${avatarId}/${MEDIA_CATEGORY_PHOTOS.SCRAPBOOK}`}>
