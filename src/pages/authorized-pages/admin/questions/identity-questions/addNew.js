...
function AddFamilyQuestion(props) {
    ...
    const onSubmit = async (data) => {
        ...
        let payloadData = JSON.stringify({
            ...
            profileQuestionBelongTo: belongTo,
        });
        ...
        try {
            ...
            const { status, data: { httpStatusCode, friendyMessageList } } = await postRequest(ADMIN.IDENTITY_QUESTION.ADD, payloadData);
            ...
            if (status === RESPONSE_CODE[200] && httpStatusCode === RESPONSE_CODE[200]) {
                ...
                reset();
                navigate(ADMIN_ROUTE_SLUGS.IDENTITY_QUESTION);
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} noValidate>
                        ...
                        <div className="col-md-2 mb-4">
                            ...
                            <select {...register('belongTo')} className="form-control">
                                <option value="" ></option>
                                {Object.keys(ENUM_AVTAR.IDENTITY_QUESTION.BELONGS_TO).map((item, index) => {
                                    return <option value={ENUM_AVTAR.IDENTITY_QUESTION.BELONGS_TO[item]} key={index}>{item.replaceAll('_', ' ')}</option>
                                })}
                            </select>
