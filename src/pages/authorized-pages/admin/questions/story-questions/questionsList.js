...
function QuestionsList() {
    ...
    async function getFamilyQuestions() {
        ...
        try {
            const { status, data: { httpResponseDetail: { httpStatusCode }, result: { data: { questionResponses } }, count }, } = await postRequest(ADMIM.STORY_QUESTION.LIST, payloadData);
            ...
            {!loader &&
                <>
                    ...
                    <ReactPaginate
                        ...
                        pageCount={pageCount}
                    />
                    <Link to={ADMIN_ROUTE_SLUGS.ADD_STORY_QUESTION} state={{ questionType: 'parent', parentQuestionId: 0 }} className="btn primary-btn btn-xs">
