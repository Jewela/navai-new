...
function AddFamilyQuestion(props) {
    ...
    const onSubmit = async (data) => {
        ...
        let payloadData = JSON.stringify({
            ...
            familyQuestionBelongTo: belongTo,
        });

        try {
            dispatch(ACTION_SERVICES(actions.API_PROCESS));
            const { status, data: { httpStatusCode, friendyMessageList } } = await postRequest(ADMIM.FAMILY_QUESTION.ADD, payloadData);
            ...
            <div className="row">
                <div className="col-md-12">
                    <div className="w-100 d-flex justify-content-between align-items-center mb-4">
                        <span className="h5 mb-0">Family Parent Questions</span>
                    </div>
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} noValidate>
                        ...
                        <div className="col-md-2 mb-4">
                            ...
                            <select {...register('repetition')} className="form-control">
                                <option value="" ></option>
                                {Object.keys(ENUM_AVTAR.FAMILY_QUESTION.REPETITION).map((item, index) => {
                                    return <option value={ENUM_AVTAR.FAMILY_QUESTION.REPETITION[item]} key={index}>{item.replace('_', ' ')}</option>
                                })}
                            </select>
                        ...
                        <div className="col-md-2 mb-4">
                            ...
                            <select {...register('belongTo')} className="form-control">
                                <option value="" ></option>
                                {Object.keys(ENUM_AVTAR.FAMILY_QUESTION.BELONGS_TO).map((item, index) => {
                                    return <option value={ENUM_AVTAR.FAMILY_QUESTION.BELONGS_TO[item]} key={index}>{item}</option>
                                })}
                            </select>
