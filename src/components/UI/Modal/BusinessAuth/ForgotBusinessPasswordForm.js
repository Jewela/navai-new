...
function ForgotBusinessPasswordForm(props) {
  ...
  const validationSchema = yup.object().shape({
    ...
    new_password: yup.string().when("email", (email, schema) => {
      return emailReadOnly
        ? schema.required("Please enter new password")
        : schema.notRequired();
    }),
    otp: yup.string().when("email", (email, schema) => {
      return emailReadOnly
        ? schema.required("Please enter one time pin")
        : schema.notRequired();
    }),
  });
  ...
  const GenerateHashPassword = async (password) => {
    ...
    try {
      const response = await postRequest(
        `${AUTH.GENERATE_HASHCODE}?password=${password}`,
        null
      );
  ...
  const onSubmit = async (formData) => {
    try {
      setLoader(true);
      if (emailReadOnly) {
        const { status: HashPasswordStatus, HashPassword } =
          await GenerateHashPassword(formData.new_password);
        if (!HashPasswordStatus) {
          toast.error(
            `Getting error while generating hash password. Please try again.`
          );
          setLoader(false);
          return false;
        }
        ...
        if (statusCode === RESPONSE_CODE[200]) {
          ...
          toast.success(
            RESPONSE_MESSAGES[DEFAULT_VALUE.LOCALE].RESET_PASS_SUCCESS
          );
          FnHandleClose();
        }
      } else {
        const response = await getRequest(
          AUTH.FORGOT_PASSWORD.replace("{EMAIL}", formData.email)
        );
        const {
          status: statusCode,
          data: { data },
...
export default ForgotBusinessPasswordForm;
