...
import actions, { FAVOURIE_AVATARS_ACTION as fav_actions } from "../../../redux/authenticate/actions";
        ...
        <div className="avator-listing">
            {
                props.avtarList.map((avtar, index) => {
                    ...
                    return <div key={`key-for-avatar-${index}-${avtar.id}`} className="avator__items position-relative">
                        <div className="avator__headshot mb-3 position-relative">
                            {isAuthenticated
                                ? <Link to={`${AUTH_ROUTE_SLUGS.AVATAR_MEETING}/${avtar.id}`}>
                                    <img src={avtar.profileImage ? avtar.profileImage : 'images/avator-4.jpg'} alt={avtar.name} className="rounded-10" />
                                </Link>
                                : <span role="button" onClick={() => handleAuth(avtar.avatarCategoryEnum, `${AUTH_ROUTE_SLUGS.AVATAR_MEETING}/${avtar.id}`)}>
                                    <img src={avtar.profileImage ? avtar.profileImage : 'images/avator-4.jpg'} alt={avtar.name} className="rounded-10" />
                                </span>
                            }
                            ...
                            <div className="avator__items__options">
                                ...
                                >
                                    {isAuthenticated
                                        ? <Link to={`${route}/${avtar.id}`} className="btn-circle btn-circle__primary">
                                            <i className="ri-links-line"></i>
                                        </Link>
                                        : <span role='button' onClick={() => handleAuth(avtar.avatarCategoryEnum, `${AUTH_ROUTE_SLUGS.AVATAR_MEETING}/${avtar.id}`)} className="btn-circle btn-circle__primary">
                                            <i className="ri-links-line"></i>
                                        </span>
                                    }
                                </OverlayTrigger>
                                ...
                                >
                                    <Link to={`${PUBLIC_ROUTES_SLUGS.AVATAR_BIO}/${avtar.id}`} className="btn-circle btn-circle__secondary" >
                        ...
                        <div className="text-white h5">{avtar.name}</div>
                    </div>
                })
            }
            <div>
